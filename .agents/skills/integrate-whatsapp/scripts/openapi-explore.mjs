...
const DEFAULT_PUBLISHED_WHATSAPP_OPENAPI =
  "https://docs.kapso.ai/api/meta/whatsapp/openapi-whatsapp.yaml";
...
function inferSpecId(filePath, spec) {
  const base = (
    isUrl(filePath) ? path.basename(new URL(filePath).pathname) : path.basename(filePath)
  ).toLowerCase();
...
function formatSchemaPreview(
  ...
  if (schema.$ref) {
    ...
    if (resolved && depth > 0) {
      ...
      const childLines = formatSchemaPreview(
        next,
        doc,
        { depth, maxProps },
        indent + "  ",
      ...
      if (!(childLines.length === 1 && childLines[0].trim() === "object"))
    ...
    for (const key of keys.slice(0, maxProps)) {
      ...
      if (depth > 0) {
        ...
        if (childType === "object" || childNorm?.properties || childNorm?.items) {
          const nested = formatSchemaPreview(
            childNorm,
            doc,
            { depth: depth - 1, maxProps },
            indent + "  ",
            refStack,
          );
          // Only include nested if it adds something beyond "object"/"array"
          if (!(nested.length === 1 && ["object", "array"].includes(nested[0].trim())))
            lines.push(...nested);
        }
      }
    }
  ...
  if (t === "array" || normalized.items) {
    ...
    if (normalized.items && depth > 0) {
      const next = normalized.items?.$ref
        ? getByJsonPointer(doc, normalized.items.$ref)
        : normalized.items;
      ...
      const redundant =
        nestedTrim === null
          ? false
          : ["object", "array"].includes(nestedTrim) || nestedTrim === itemType;
  ...
  if (normalized.oneOf?.length) {
    const opts = normalized.oneOf
      .slice(0, 5)
      .map((s) => schemaTypeString(s))
      .join(" | ");
    return [`${indent}oneOf(${opts}${normalized.oneOf.length > 5 ? " | ..." : ""})`];
  }
  if (normalized.anyOf?.length) {
    const opts = normalized.anyOf
      .slice(0, 5)
      .map((s) => schemaTypeString(s))
      .join(" | ");
...
function exampleFromSchema(
  ...
  if ((t === "array" || normalized.items) && depth > 0) {
    return [
      exampleFromSchema(normalized.items || {}, doc, { depth: depth - 1, includeOptional }, stack),
    ];
  }
  ...
  if (normalized.oneOf?.length)
    return exampleFromSchema(normalized.oneOf[0], doc, { depth, includeOptional }, stack);
  if (normalized.anyOf?.length)
    return exampleFromSchema(normalized.anyOf[0], doc, { depth, includeOptional }, stack);
...
async function loadSpecsAsync({ sources, sourceMode, specFilter }) {
  ...
  for (const filePath of sources) {
    ...
    for (const [p, pathItemRaw] of Object.entries(paths)) {
      ...
      const pathParams = Array.isArray(pathItem.parameters)
        ? pathItem.parameters.map((x) => derefObject(x, doc))
        : [];
  ...
  if (!specs.length) {
    if (sourceMode === "default_remote" && errors.length) {
      ...
      if (local.length) {
        console.error(
          `Could not fetch published OpenAPI specs. Falling back to local repo specs (api/**/openapi-*.yaml).`,
        );
...
function printSpecs(specs, { json } = {}) {
  ...
  for (const s of specs)
    rows.push([
      ...
      String(Object.keys(s.schemas).length),
      s.filePath,
    ]);
  printTable(rows);
}
...
function printTags(specs, { specHint } = {}) {
  ...
  for (const s of specs) {
    ...
    for (const t of s.tags || [])
      rows.push([s.id, t.name, (t.description || "").replaceAll("\n", " ").slice(0, 80)]);
  }
...
function printOps(specs, { specHint, tag, q, json, limit = 30 } = {}) {
  ...
  if (q) {
    ...
    const opSet = new Set(
      res.ops.map((o) => `${o.specId}:${o.operationId || o.method + " " + o.path}`),
    );
...
function printOperation(op, spec) {
  ...
  const id = op.operationId
    ? `${op.specId}:${op.operationId}`
    : `${op.specId}:${op.method.toUpperCase()} ${op.path}`;
  ...
  if (op.responses && Object.keys(op.responses).length) {
    ...
    for (const code of codes) {
      ...
      const desc = resp?.description
        ? ` - ${String(resp.description).replaceAll("\n", " ").slice(0, 80)}`
        : "";
      console.log(`\n${code}${desc}`);
  ...
  const preferred =
    candidates.find((req) =>
      Object.keys(req).some(
        (name) => schemes[name]?.type === "apiKey" && schemes[name]?.in === "header",
      ),
    ) || candidates[0];
...
function printSchemaDetails(match, specs, { includeUsedBy = true } = {}) {
  ...
  if (includeUsedBy) {
    ...
    if (usedBy.length) {
      ...
      for (const op of usedBy.slice(0, 30))
        rows.push([op.specId, op.operationId || "-", op.method.toUpperCase(), op.path]);
      printTable(rows);
...
function printWhere(specs, schemaQuery, { specHint, json, limit = 30 } = {}) {
  ...
  if (matches.length > 1 && !schemaQuery.includes(":")) {
    die(
      `Schema exists in multiple specs. Use specId:SchemaName\n${matches.map((m) => `- ${m.spec.id}:${m.name}`).join("\n")}`,
    );
  }
  ...
  const rows = [["spec", "operationId", "method", "path", "summary"]];
  for (const o of usedBy.slice(0, limit))
    rows.push([
      o.specId,
      o.operationId || "-",
      o.method.toUpperCase(),
...
async function main() {
  ...
  if (command === "op") {
    ...
    if (
      matches.length > 1 &&
      !q.includes(":") &&
      !q.match(/^(get|post|put|patch|delete|options|head)\\s+/i)
    ) {
      die(
        `OperationId exists in multiple specs. Use specId:operationId\n${matches.map((m) => `- ${m.specId}:${m.operationId}`).join("\n")}`,
      );
    }
    const op = matches[0];
