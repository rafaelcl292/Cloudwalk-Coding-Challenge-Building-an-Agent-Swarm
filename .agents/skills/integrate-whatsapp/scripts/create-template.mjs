...
function usage() {
  return {
    usage:
      "node scripts/create-template.mjs --business-account-id <WABA_ID> --json <payload> | --file <path>",
...
async function main() {
  ...
  try {
    ...
    return printResult(
      err("Failed to create template", { message: String(error?.message || error), ...usage() }),
    );
  }
}
