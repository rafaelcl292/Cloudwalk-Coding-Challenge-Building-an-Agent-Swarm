...
function usage() {
  return {
    usage:
      "node scripts/list-templates.mjs --business-account-id <WABA_ID> [--name <name>] [--status <status>] [--category <category>] [--language <lang>] [--fields <fields>] [--limit <n>] [--after <cursor>] [--before <cursor>] [--query <json>]",
...
async function main() {
  ...
  try {
    const businessAccountId = requireFlag(
      flags,
    ...
    return printResult(
      err("Failed to list templates", { message: String(error?.message || error), ...usage() }),
    );
  }
}
