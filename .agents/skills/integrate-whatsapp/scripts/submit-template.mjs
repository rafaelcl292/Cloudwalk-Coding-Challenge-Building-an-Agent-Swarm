...
function usage() {
  return {
    usage:
      "node scripts/submit-template.mjs --business-account-id <WABA_ID> --json <payload> | --file <path>",
...
async function main() {
  ...
  try {
    ...
    return printResult(
      err("Failed to submit template", { message: String(error?.message || error), ...usage() }),
    );
  }
}
