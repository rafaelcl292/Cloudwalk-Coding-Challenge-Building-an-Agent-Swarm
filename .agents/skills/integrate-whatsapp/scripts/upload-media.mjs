...
function usage() {
  return {
    usage:
      "node scripts/upload-media.mjs --phone-number-id <PHONE_NUMBER_ID> --file <path> --mime-type <mime> [--filename <name>]",
...
async function main() {
  ...
  try {
    const phoneNumberId = requireFlag(
      flags,
    ...
    return printResult(
      err("Failed to upload media", { message: String(error?.message || error), ...usage() }),
    );
  }
}
