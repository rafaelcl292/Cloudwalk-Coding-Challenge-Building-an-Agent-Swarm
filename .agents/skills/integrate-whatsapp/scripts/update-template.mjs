...
function usage() {
  return {
    usage:
      "node scripts/update-template.mjs --business-account-id <WABA_ID> --hsm-id <template_id> --json <payload> | --file <path>",
...
async function main() {
  ...
  try {
    ...
    return printResult(
      err("Failed to update template", { message: String(error?.message || error), ...usage() }),
    );
  }
}
