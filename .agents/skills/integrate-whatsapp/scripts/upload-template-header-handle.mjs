...
function usage() {
  return {
    usage:
      "node scripts/upload-template-header-handle.mjs --file <path> --mime-type <mime> --resumable-app-id <app_id>",
...
function main() {
  try {
    return printResult(
      blocked(
        "Meta proxy does not expose the resumable upload endpoints required for header_handle.",
        {
          missing_endpoints: [
    ...
    return printResult(
      err("Failed to explain header handle limitations", {
